...
**面试话术**:
> "首次查询需要经过 Embedding + 向量检索 + LLM 结构化抽取,整个链路约 10-25 秒。这个延迟短期内无法消除,但**用户感知到的等待时间是可以优化的**。
> 
> 我实现了一个**渐进式加载步骤组件**,在等待期间实时展示系统正在执行的阶段:匹配产品 → 检索条款 → AI 深度分析 → 生成卡片。每个阶段有独立进度条和状态图标。
> 
> 这背后的设计原理是**感知性能工程**:心理学研究表明,有进度反馈的等待比无反馈的等待,用户感知时间短约 36%。同时底部提示'首次分析需要 AI 深度提取,结果将自动缓存',给用户**明确的预期管理**——告诉他这是一次性成本。
> 
> 这体现了一个重要的产品原则:**当你无法让系统更快时,就让等待变得可以忍受**。"

**追问应对**:
- **Q: 为什么不用流式输出(Streaming)?**
  A: 我们的场景是结构化 JSON 抽取,不是逐字生成的对话。JSON 必须完整返回后才能解析渲染为卡片,流式输出在这个场景下没有意义。渐进式加载步骤是更合适的方案。

- **Q: 进度是真实的还是模拟的?**
  A: 步骤时长基于实际观测的各阶段耗时分布,是接近真实的模拟。完全真实的进度需要后端发送 SSE 事件,对 MVP 阶段来说 ROI 不高。当前方案用最小成本解决了 90% 的体验问题。
...
**Q: pgvector 的索引策略是什么?**
> "使用 HNSW 索引,平衡了检索速度和准确率。系统采用**两阶段延迟模型**:首次查询经混合检索 + LLM 抽取后自动写入缓存;后续命中缓存的请求 P95 延迟 ≤3000ms,实测通常 <100ms。这种设计将 LLM 调用成本均摊到首次请求,大幅提升整体吞吐效率。"
...
> 4. **P95 延迟 (≤3000ms)**: 验证缓存命中后的性能可预测性
> 5. **错误率 (≤5%)**: 验证系统稳定性
