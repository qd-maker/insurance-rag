...
export default function App() {
  ...
  const handleSearch = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      // 尝试 V2 Advanced RAG Pipeline API
      let data: any = null;
      try {
        const v2Res = await fetch('/api/v2/search', {
          method: 'POST',
          headers: new Headers({ 'Content-Type': 'application/json' }),
          body: JSON.stringify({ query: fullQuery, streaming: false, debug: false }),
        });
        if (v2Res.ok) {
          data = await v2Res.json();
        }
      } catch {
        // V2 失败，静默 fallback 到 V1
      }
